...
class OptimizerFonts extends OptimizerBase
{
    ...
    private function injectLinks($preloadType, $links)
    {
        if (in_array($preloadType, self::TWO_PRELOAD_LINK_TYPES, false)) { // phpcs:ignore
            ...
            if (!empty($preloadString)) {
                $this->content = OptimizerUtils::inject_in_html($this->content,$preloadString, array( '<head', 'after_tag' ));
            }
        }
    }
    ...
    public function combineGoogleFonts()
    {
        ...
        if(count($fonts)>1){
            ...
            $this->content = OptimizerUtils::inject_in_html($this->content,$google_font_tag, array( '<head', 'after_tag' ));
    ...
    public function addPreconnect()
    {
        ...
        $this->content = OptimizerUtils::inject_in_html($this->content,$preconnect_string, array( '<head', 'after_tag' ));
...
<noscript>
    ...
    public function add_WebFont(){
        ...
        ";
        $this->content = OptimizerUtils::inject_in_html($this->content,$webFont_js, array( '<head', 'after_tag' ));
    }
